    ...
    )
{
    ...
    error_code = ioctl(drive_ptr->DEV_FILE_PTR, IO_IOCTL_GET_BLOCK_SIZE, &sector_size);// 获取 BLOCK_SIZE
#if !MQX_USE_IO_OLD
    if (error_code == -1) {
        error_code = errno;
    }
    ...
    if (error_code == MFS_NO_ERROR) {
        error_code = ioctl(drive_ptr->DEV_FILE_PTR, IO_IOCTL_GET_NUM_SECTORS, &num_sectors);// 获取 NUM_SECTORS
#if !MQX_USE_IO_OLD
        if (error_code == -1) {
            error_code = errno;
        }
