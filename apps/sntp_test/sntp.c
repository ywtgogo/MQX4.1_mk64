...
#define C_ADDR             0xC0A80106
#define D_ADDR             0x0AC0E1DE
...
static uint32_t SNTP_timed_send_recv(uint32_t sock, _ip_address *, uint32_t *);
...
extern void Shell_Task(uint32_t param);
#define INIT_TASK_ID  1
#define SHELL_TASK_ID 2
TASK_TEMPLATE_STRUCT  MQX_template_list[] = 
{
    /* Task Index,  Function,       Stack,  Priority,   Name,       Attributes,             Param,  Time Slice */
    { INIT_TASK_ID,            Init_Task,     1000,  12,         "inittask",   MQX_AUTO_START_TASK,    0,      0 },
    { SHELL_TASK_ID,            Shell_Task,    2500,   14,         "Shell",      0,       0,      0 },
    { 0 }
};
...
{ /* Body */
   ...
   */
   ...
   SNTP_add96(temp1, temp2, t4);
   t4[1] = t4[1] - 2208988800u;
   t4[2] = t4[2] / (0xFFFFFFFF / 1000);
   _time_to_date((TIME_STRUCT_PTR)(&t4[1]),&clk_time);
   printf("Year=%d, Month=%d, day= %d, hour=%d, min=%d,sec=%d\n",clk_time.YEAR,  \
   clk_time.MONTH,clk_time.DAY,clk_time.HOUR,clk_time.MINUTE,clk_time.SECOND);
...
int32_t Shell_Ntp(int32_t argc, char *argv[] )
{
    //SNTP_init("sntp",2,1500,D_ADDR,5000);
    SNTP_oneshot(D_ADDR,1500);
