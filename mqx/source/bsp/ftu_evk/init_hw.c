...
/*FUNCTION*---------------------------------------------------------------------
*
* Function Name    : _bsp_flexbus_dm9000_setup
* Returned Value   :
* Comments         :
*    Setup FlexBus for DM9000 operation
*
*END*-------------------------------------------------------------------------*/
void _bsp_flexbus_dm9000_setup (uint8_t fb_cs, const uint32_t base_address)
{
    FB_MemMapPtr fb_ptr = FB_BASE_PTR;

    /* Enable external MRAM mapped on CS0 */
    /* CS0 base address */
    fb_ptr->CS[fb_cs].CSAR = base_address;
    /* CS0 control (8bit data, 1 wait state) */
    fb_ptr->CS[fb_cs].CSCR = FB_CSCR_PS(1) |
                         FB_CSCR_AA_MASK | 
                         FB_CSCR_WS(1)   | 
                         FB_CSCR_BEM_MASK; 

    /* CS0 address mask and enable */
    fb_ptr->CS[fb_cs].CSMR = FB_CSMR_V_MASK;
}

/*FUNCTION*---------------------------------------------------------------------
*
* Function Name    : _bsp_ext_watchdog_kick
* Returned Value   : void
* Comments         :
*   Kick external watch dog, it is expected that this funtion should be called within every 1.6s.
*
*END*-------------------------------------------------------------------------*/
void _bsp_ext_watchdog_kick(void)
{
	static bool is_first_time = TRUE;

	if(is_first_time){
		PORT_PCR_REG(BSP_EXT_WATCHDOG_WDI_PORT, BSP_EXT_WATCHDOG_WDI_PIN) = PORT_PCR_MUX(0x01) | PORT_PCR_PE_MASK;
    		GPIO_PDDR_REG(PTA_BASE_PTR) |= 1 <<BSP_EXT_WATCHDOG_WDI_PIN; 
    		GPIO_PDOR_REG(PTA_BASE_PTR) |= 1 << BSP_EXT_WATCHDOG_WDI_PIN; 	/* Output high level */
		is_first_time = FALSE;
	}

	GPIO_PTOR_REG(PTA_BASE_PTR) |= 1 << BSP_EXT_WATCHDOG_WDI_PIN;
}
