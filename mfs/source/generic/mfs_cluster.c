    ...
    )
{
    ...
    if ( (cluster_number >= CLUSTER_MIN_GOOD) && (cluster_number <= drive_ptr->LAST_CLUSTER) )
    {
        do
        {
            ...
            del_info[0] = first_sector_in_cluster;
	    del_info[1] = drive_ptr->BPB.SECTORS_PER_CLUSTER;
	    _io_ioctl(drive_ptr->DEV_FILE_PTR, IO_IOCTL_DELETE_SECTORS, del_info);
	...
	_io_ioctl(drive_ptr->DEV_FILE_PTR, IO_IOCTL_DELETE_DONE, NULL);
    }
    else
    {
