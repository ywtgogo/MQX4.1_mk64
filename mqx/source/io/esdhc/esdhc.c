    ...
    )
{
    ...
    _esdhc_set_baudrate_low(esdhc_ptr, ( esdhc_init_ptr->CLOCK_SPEED ), baudrate);//_bsp_get_clock_freq
    ...
    )
{
    ...
    return _esdhc_get_baudrate_low(esdhc_ptr, ( esdhc_init_ptr->CLOCK_SPEED ) );//_bsp_get_clock_freq
}
...
#define CARD_MP_MASK            0x20
  ...
  if (card_type | CARD_MP_MASK)
  {
      ...
      command.BLOCKS = 0;
      val = _esdhc_send_command(esdhc_device_ptr, &command, NULL);
      if ( (val > 0) && (val != ESDHC_ERROR_COMMAND_TIMEOUT) )
      {
    	  return ESDHC_ERROR_INIT_FAILED;
      }
      ...
      if ( (val < 0) || (val == ESDHC_ERROR_COMMAND_TIMEOUT) )
      {
          ...
          command.BLOCKS = 0;
          val = _esdhc_send_command(esdhc_device_ptr, &command, NULL);
          if (ESDHC_ERROR_COMMAND_FAILED != val)
          {
        	  if( val )
        	  {
        		  return ESDHC_ERROR_INIT_FAILED;
    ...
    )
{
	...
	while(esdhc_ptr->PRSSTAT & SDHC_PRSSTAT_DLA_MASK)  // pbanta
	{}                                                      // pbanta
    ...
    {
        /* Something is wrong - timeout or any error of light weight event */
    	esdhc_fd_ptr->ERROR = IO_ERROR_TIMEOUT;
        return IO_ERROR;
    }
    else
    {
