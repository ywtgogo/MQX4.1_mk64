...
int32_t MFS_filesync(MQX_FILE_PTR fd_ptr)
{
	MFS_DRIVE_STRUCT_PTR    drive_ptr = fd_ptr->DEV_PTR->DRIVER_INIT_PTR;
    MFS_HANDLE_PTR          handle_ptr = fd_ptr->DEV_DATA_PTR;;
    TIME_STRUCT             time;
...
/*	
    if(handle_ptr->TOUCHED)
    {
        ...
        htods(handle_ptr->DIR_ENTRY.TIME, PACK_TIME(clk_time));
        htods(handle_ptr->DIR_ENTRY.DATE, PACK_DATE(clk_time));
        error_code = MFS_Update_entry(drive_ptr, handle_ptr);
    }
...
*/	
	...
	//_io_ioctl(drive_ptr->DEV_FILE_PTR, IO_IOCTL_FLUSH_DRIVE, NULL);

	//_io_ioctl(fd_ptr, IO_IOCTL_FLUSH_OUTPUT, NULL); 
        
        return error_code;
}

int32_t MFS_get_freesize(MQX_FILE_PTR fd_ptr)
{
    MFS_DRIVE_STRUCT_PTR    drive_ptr = fd_ptr->DEV_PTR->DRIVER_INIT_PTR;
    uint32_t                 sector_number;
    
    _io_ioctl(drive_ptr->DEV_FILE_PTR, IO_IOCTL_GET_FREEPAGES, &sector_number);
