...
{ /* Body */
   ...
   int_io_dev_ptr = (void *)io_dev_ptr->DRIVER_INIT_PTR;
   flags          = fd_ptr->FLAGS;
   io_info_ptr = (KUART_INFO_STRUCT_PTR)int_io_dev_ptr->DEV_INFO_PTR;
      ...
      if (flags & IO_SERIAL_TRANSLATION) {
         if (c == '\r') {
            ...
            if (flags & IO_SERIAL_ECHO) {
                #if BSP_RS485_EN 
               _bsp_serial_rs485_tx_enable(io_info_ptr->INIT.DEVICE, TRUE);
               #endif
               _io_serial_int_putc_internal(int_io_dev_ptr, (char)c, 0);
               while((sci_ptr->S1 & UART_S1_TC_MASK) == 0);
               #if BSP_RS485_EN   
               _bsp_serial_rs485_tx_enable(io_info_ptr->INIT.DEVICE, FALSE);
               #endif
            } /* Endif */
            /* End CR 387 */
            c = '\n';
...
{ /* Body */
   ...
   switch (cmd) {
	  ...
	  case IO_IOCTL_SERIAL_GET_LEN:
		result = _CHARQ_SIZE(int_io_dev_ptr->IN_QUEUE);
		break;

	  case IO_IOCTL_SERIAL_CLEAR_BUF:
		_CHARQ_INIT(int_io_dev_ptr->IN_QUEUE,int_io_dev_ptr->IN_QUEUE->MAX_SIZE);
		result = MQX_OK;
		break;
