...
)
{
    ...
    PORT_MemMapPtr  pctl;
    PORT_MemMapPtr  pctlNew;

    /* Setup GPIO for UART devices */
    switch (dev_num)
    {
        case 0:
            //pctl = (PORT_MemMapPtr)PORTA_BASE_PTR;//TX ptd7 RX ptd6
        ...
        case 1:
            ...
            pctlNew = (PORT_MemMapPtr)PORTE_BASE_PTR;
            if (flags & IO_PERIPHERAL_PIN_MUX_ENABLE)
            {
                ...
                pctlNew ->PCR[0] = 0 | PORT_PCR_MUX(3) | PORT_PCR_DSE_MASK;
            }
            if (flags & IO_PERIPHERAL_PIN_MUX_DISABLE)
            {
                /* PTC3 default */
        ...
        case 3:
            ...
            if (flags & IO_PERIPHERAL_PIN_MUX_ENABLE)
            {
                /* PTC17 as RX function (Alt.3) + drive strength */
                pctl->PCR[17] = 0 | PORT_PCR_MUX(3) | PORT_PCR_DSE_MASK;
                /* PTC16 as TX function (Alt.3) + drive strength */
        ...
        case 4:
            ...
            if (flags & IO_PERIPHERAL_PIN_MUX_ENABLE)
            {
                ...
                /* PTE11 as RX485I DIR pin */
                pctl->PCR[11] = 0 | PORT_PCR_MUX(1);
                GPIOE_PDDR |= 1<<11;
                GPIOE_PSOR |= 1<<11;
        ...
        case 5:
            ...
            if (flags & IO_PERIPHERAL_PIN_MUX_ENABLE)
            {
                ...
                /* PTE10 as RX485II DIR pin */
                pctl->PCR[10] = 0 | PORT_PCR_MUX(1);
                GPIOE_PDDR |= 1<<10;
                GPIOE_PSOR |= 1<<10;
            }
            if (flags & IO_PERIPHERAL_PIN_MUX_DISABLE)
...
/*FUNCTION*-------------------------------------------------------------------
*
* Function Name    : _bsp_serial_rs485_tx_enable
* Returned Value   :
* Comments         : flag = 0, T mode  flag = 1, R mode
*    This function performs BSP-specific initialization related to rs485
*
*END*----------------------------------------------------------------------*/

void _bsp_serial_rs485_tx_enable
(
    /* [IN] Serial device number */
    uint8_t dev_num,
    uint8_t flag
...
)
{
    ...
    switch (dev_num)
    {
        ...
        case 1:
            //pctl = (PORT_MemMapPtr)PORTE_BASE_PTR; //1\2\3\4
            pctl = (PORT_MemMapPtr)PORTB_BASE_PTR;
...
_mqx_int _bsp_i2c_io_init(uint32_t dev_num) {
   ...
   case 0:
      ...
      pctl->PCR[2] = PORT_PCR_MUX(2) | PORT_PCR_ODE_MASK;
      pctl->PCR[3] = PORT_PCR_MUX(2) | PORT_PCR_ODE_MASK;
...
/*FUNCTION*-------------------------------------------------------------------
*
* Function Name    : _bsp_dm9000_io_init
* Returned Value   : MQX_OK or -1
* Comments         :
*    This function performs BSP-specific initialization related to ENET
*
...
)
{
    ...
    /* DM9000 IO */
    pctl = (PORT_MemMapPtr)PORTB_BASE_PTR;
    pctl->PCR[19] = PORT_PCR_MUX(5); /* PTB19, FB_OE_b, IOR */
    pctl->PCR[20] = PORT_PCR_MUX(5); /* PTB20, FB_AD31, SD7 */
    pctl->PCR[21] = PORT_PCR_MUX(5); /* PTB21, FB_AD30, SD6 */
    pctl->PCR[22] = PORT_PCR_MUX(5); /* PTB22, FB_AD29, SD5 */
    pctl->PCR[23] = PORT_PCR_MUX(5); /* PTB23, FB_AD28, SD4 */

    pctl = (PORT_MemMapPtr)PORTC_BASE_PTR;
    pctl->PCR[11] = PORT_PCR_MUX(5); /* PTC11, FB_RW_b, IOW */
    pctl->PCR[12] = PORT_PCR_MUX(5); /* PTC12, FB_AD27, SD3 */
    pctl->PCR[13] = PORT_PCR_MUX(5); /* PTC13, FB_AD26, SD2 */
    pctl->PCR[14] = PORT_PCR_MUX(5); /* PTC14, FB_AD25, SD1  */
    pctl->PCR[15] = PORT_PCR_MUX(5); /* PTC15, FB_AD24, SD0 */

    pctl = (PORT_MemMapPtr)PORTD_BASE_PTR;
    pctl->PCR[0] = PORT_PCR_MUX(5); /* PTD0, FB_CS1_b, CS */
    pctl->PCR[5] = PORT_PCR_MUX(5); /* PTD5, FB_AD1, CMD */
    pctl->PCR[8] = PORT_PCR_PS_MASK | PORT_PCR_PE_MASK | PORT_PCR_IRQC(10) |
                    PORT_PCR_PFE_MASK | PORT_PCR_MUX(1); /* PTD8, DM9000_INT, INT */

    /* Enable clock for dm9000 module */
    sim->SCGC5 |= SIM_SCGC5_PORTD_MASK;
    sim->SCGC7 |= SIM_SCGC7_FLEXBUS_MASK;
    sim->SOPT2 |= SIM_SOPT2_FBSL(0x3);
